...
void CNetVarManager::Initialize()
{
	...
	ClientClass* clientClass = g_pInterfaces->Client->GetAllClasses();
	if (!clientClass)
		return;
...
{
	m_file.open("C:/Netvars.txt");

	for (ClientClass* pClass = g_pInterfaces->Client->GetAllClasses(); pClass != NULL; pClass = pClass->m_pNext)
	{
