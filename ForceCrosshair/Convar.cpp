...
void ConVar::m_nValue(const char* value)
{
	typedef void(__thiscall* OriginalFn)(void*, const char*);
	return g_pClientInfo->GetVFunc<OriginalFn>(this, 14)(this, value);
}

void ConVar::m_nValue(float value)
{
	typedef void(__thiscall* OriginalFn)(void*, float);
	return g_pClientInfo->GetVFunc<OriginalFn>(this, 15)(this, value);
}

void ConVar::m_nValue(int value)
{
	typedef void(__thiscall* OriginalFn)(void*, int);
	return g_pClientInfo->GetVFunc<OriginalFn>(this, 16)(this, value);
}

void ConVar::m_nValue(char value)
{
	typedef void(__thiscall* OriginalFn)(void*, Color);
	return g_pClientInfo->GetVFunc<OriginalFn>(this, 17)(this, value);
}

char* ConVar::GetName()
{
	typedef char*(__thiscall* OriginalFn)(void*);
	return g_pClientInfo->GetVFunc<OriginalFn>(this, 5)(this);
}
...
{
	m_pOriginalCVar = g_pInterfaces->Convar->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::~SpoofedConvar()
{
	if (IsSpoofed())
	{
		...
		g_pInterfaces->Convar->UnregisterConCommand(m_pDummyCVar);
		free(m_pDummyCVar);
...
void SpoofedConvar::Spoof()
{
	if (!IsSpoofed() && m_pOriginalCVar)
	{
		...
		g_pInterfaces->Convar->RegisterConCommand(m_pDummyCVar);
...
{
	if (IsSpoofed())
		m_pOriginalCVar->m_nValue(iValue);
}

void SpoofedConvar::SetBool(bool bValue)
{
	if (IsSpoofed())
		m_pOriginalCVar->m_nValue(bValue);
}

void SpoofedConvar::SetFloat(float flValue)
{
	if (IsSpoofed())
		m_pOriginalCVar->m_nValue(flValue);
}

void SpoofedConvar::SetString(const char* szValue)
{
	if (IsSpoofed())
		m_pOriginalCVar->m_nValue(szValue);
}