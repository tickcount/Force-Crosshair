...
public:
	...
	HANDLE WINAPI MakeThread(EXECUTE* Thread)
	{
		return CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)Thread, NULL, NULL, NULL);
	}

	BOOL WINAPI DisableTLC(HMODULE Libary)
	{
		return DisableThreadLibraryCalls(Libary);
	}
	...
	void RemovePE(HMODULE Module)
	{
		DWORD ModuleBase = DWORD(Module);
	...
	bool HideThread(HANDLE hThread)
	{
		...
		hThread == NULL ? Status = NtSIT(GetCurrentThread(), 0x11, 0, 0) : Status = NtSIT(hThread, 0x11, 0, 0);
		return Status != 0x00000000 ? false : true;
	}

	static HMODULE HModuleAdress()
	{
		MEMORY_BASIC_INFORMATION info;
		size_t len = VirtualQueryEx(GetCurrentProcess(), (void*)HModuleAdress, &info, sizeof(info));
		Assert(len == sizeof(info));
		return len ? (HMODULE)info.AllocationBase : NULL;
	}

	HANDLE WINAPI UniqueThread(EXECUTE* Thread, HINSTANCE HModule, BOOL Protect = false)
	{
		g_pGlobals->HModule = HModule;

		if (Protect)
		{
			this->HideThread(NULL);
			this->DisableTLC(HModule);

			this->HideModule(HModule);
			this->HideFromPEB(HModule);

			this->RemoveHeader(HModule);
			this->RemovePE(HModuleAdress());
		}

		this->MakeThread(Thread);
		return HANDLE();
	}

}; CThreading* Threading = new CThreading();